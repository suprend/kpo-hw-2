...
- Приложение — текстовый интерфейс для управления банковскими счетами, категориями и финансовыми операциями. Пользователь может создавать, редактировать, удалять и просматривать сущности, фильтровать операции, импортировать и экспортировать данные в JSON/YAML/CSV.
- Встроенная аналитика (`internal/application/analytics`) считает суммарные доходы/расходы и чистый итог по списку операций; экран списка отображает эти значения в шапке.
...
- Bootstrap (`internal/infrastructure/di/bootstrap/*.go`) разбит на слои:
  ...
  - `registerApplication` — фасады, сервисы импорт/экспорт и аналитика;
  - `registerCommands` — сервисы команд с декораторами (включая аналитические команды);
...
- Сборка и запуск:
  ...
  go run ./cmd/finance
  ```
- Логи таймингов пишутся в `cmd/finance/logs/timings.log`. Каталог `logs` создаётся рядом с бинарником (в рабочей директории запуска).
- Экспортированные файлы — JSON, YAML или CSV; импорт работает с теми же форматами.
- Экран списка операций дополнительно показывает агрегированные суммы доходов, расходов и чистый итог, рассчитываемые аналитическим сервисом.
